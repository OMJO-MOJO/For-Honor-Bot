...
class CommandHandler {
   ...
   private _emojis: Collection<string, GuildEmoji> | null | undefined = null;

   constructor(instance: WOK, commandsDir: string, client: ForHonorBot) {
   ...
   public async runCommand(
      ...
      // Get Bump4You's emojis from the support server
      if (!this._emojis) {
         this._emojis = await command.instance.client.guilds.cache
            .get("1088505948986949814")
            ?.emojis.fetch()
            .then((emojis) => emojis)
            .catch(() => null);
      }
      ...
      for (const validation of this._validations) {
         if (
            !(await validation(command, usage, this._prefixes.get(guild?.id)))
         ) {
            return;
         }
      }
